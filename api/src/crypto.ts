...
// Per-enrollment claim (D1, revised in Wave 2/A1-A2 after PR #2 review
// finding P1): the claim is generated by the ENROLLING AGENT itself, not
// this server — the API only ever stores hashSecret(claim)
// (servers.enrollment_claim_hash) and never generates or rotates a
// claim value server-side. That's the fix for the takeover found in
// review: a server-generated/rotated claim handed back in the POST
// /enroll response was itself stealable by anyone who knew the
// fleet-wide bootstrap secret plus the victim's serverId. Proves
// "you're the specific agent that enrolled THIS server id", independent
// of the bootstrap secret every agent holds. See routes/enroll.ts and
// repository.upsertPendingServer for the claim-gated re-enroll logic
// this enables.
